...
def score_v22(rec, kline, idx, sector_zt_count_today, market_zt_today=30):
    ...
    if vol_ratio < 0.7: sc["volume"] = 5
    elif vol_ratio <= 1.5: sc["volume"] = 4
    elif vol_ratio <= 2.5: sc["volume"] = 2
    else: sc["volume"] = 0

    # ─── ⑦ 大盘情绪 (10分 新增) ───
    market_zt = market_zt_today
    ft["market_zt"] = market_zt
    if market_zt >= 60: sc["emotion"] = 10
    elif market_zt >= 40: sc["emotion"] = 8
    elif market_zt >= 25: sc["emotion"] = 5
    elif market_zt >= 15: sc["emotion"] = 2
    else: sc["emotion"] = 0
...
def run(days_back, end_date):
    ...
    for code, items in by_code.items():
        ...
        for d,name,rec in items:
            ...
            sc = score_v22(rec, kl, i, sector_zt, daily_zt_count.get(d, 30))
...
def analyze(samples, end_date):
    ...
    p_json = OUT_DIR / f"v23-results-{end_date}.json"
...
def write_md(samples, n, pr, rate, avgo, avgc, avgh, buckets, dims, thresholds, top_hit, top_miss, end_date):
    p = OUT_DIR / f"v23-results-{end_date}.md"
    md=[]
    ...
    md.append("| 日期 | 代码 | 名称 | 总分 | 形 | 资 | 量价 | 辨 | 盘 | 量 | 情 | 板 | 次日收 |")
    md.append("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for s in top_hit:
        sc = s["scores"]
        md.append(f"| {s['date']} | {s['code']} | {s['name']} | **{s['total']}** | {sc.get('form',0)} | {sc.get('fund_trap',0)} | {sc.get('vol',0)} | {sc.get('distinct',0)} | {sc.get('cap',0)} | {sc.get('volume',0)} | {sc.get('emotion',0)} | {sc.get('sector',0)} | {s['next_close']:+.2f}% |")
