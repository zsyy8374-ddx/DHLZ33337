#!/usr/bin/env python3
"""reversal_lr_v4.py — 修复 v0.3 泄漏后的真实模型"""
import json, math
...
def main():
    ...
    with open(src, encoding="utf-8") as f:
        d = json.load(f)
    events = d["events"]
    print(f"📊 v0.4: {len(events)} 个事件 (基础胜率 {sum(1 for e in events if e['outcome']=='reversal')/len(events)*100:.1f}%)\n", flush=True)
    ...
    print(f"📅 v0.4 时序 AUC: {ts_avg:.4f} (vs v0.3 假 0.7976, v0.2 0.7431)", flush=True)
    print(f"📅 v0.4 Top 10% 命中: {top10_avg*100:.1f}%", flush=True)
    ...
    save_path = BACKTEST_DIR / f"reversal-lr-{today}-v4.json"
