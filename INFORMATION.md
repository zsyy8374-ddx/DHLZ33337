...
## Email Workflow
You have access to Dengxian's email accounts via Node.js scripts in the `gmail/` directory.
...
- `1628354330@qq.com` (QQ Mail SMTP) — Dengxian's primary personal mailbox
...
- **DEFAULT (since 2026-04-26): `1628354330@qq.com` via `qq-send.js`** — use for ALL outgoing email unless Dengxian says otherwise.
- **Fallback to `hello@dongshi.me`** when:
  ...
  - Dengxian explicitly requests it (“用 dongshi.me 发” / “用 Gmail 发”)
  - Sending in a professional/business context where the domain matters
- Self-sends (Dengxian → Dengxian) under 3MB: no confirmation needed.
...
- **Draft Template:** `Dear [Name], ... Cheers, Dengxian`
