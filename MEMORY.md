...
## ⚠️ Token 效率规则 (Dengxian 5-3 警告, 这次烧太多)

### 浪费的模式 (以后不要做)
1. **碎片 poll/exec** — 用 `yieldMs: 60000` 一次等够, 不要每 5 秒 poll 一次
2. **试错式编程** — 写代码前想清楚字段名, 不要靠 "改→跑→失败→改" 循环
3. **冗长汇报** — 不要每次都贴大表格, 关键数字一行带过
4. **无用的确认输出** — `echo "✅ 完成"` `cat README` 都是浪费
5. **重复读同一文件** — 一次性 `read` 够多行, 别 read 5 次
6. **过度道歉 / 总结** — Dengxian 要的是 done, 不是过程

### 应该的模式
1. **批量并行** — 多个独立 exec/edit 一次发, 不要串行
2. **写代码前先验数据** — 看 1 次 csv 列名, 比试错 5 次便宜
3. **回复极简** — 数字 + 1 句结论, 没必要的不说
4. **NO_REPLY 友好** — 不需要回复时直接 NO_REPLY, 别凑字数
